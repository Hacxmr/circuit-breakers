...
def generate_crescendo_step(round_num, task, history_a, last_response, attacker_generate):
    ...
    # Handle case where JSON parsing failed - use actor_attack style robust fallback
    if isinstance(data, str):
        ...
        if not fallback_question:
            fallback_question = f"Can you provide more information about {task[:50]}?"
    ...
    if not question or not isinstance(question, str):
        question = f"Can you help me understand more about {task[:50]}?"
    if not summary or not isinstance(summary, str):
        summary = "Previous response contained relevant information"
        
    return question, summary

def run_crescendomation(test_case, output_file_path, target_generate, attacker_generate, args, integration=None):
    goal = test_case["task"]
